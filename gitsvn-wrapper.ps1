...
# Execute
if ($options -eq "") {
    $result = $wrapper.Execute($url, $username, $password, $certAcceptResponse, $timeout)
} else {
    $result = $wrapper.Execute($url, $username, $password, $certAcceptResponse, $timeout, $options)
}
